...
class HierarchicalCatalog(object):
        ...
        if not self.catalogs[1]:
            for freq in self.structure.keys(): 
                ...
                if os.path.exists(target_file):
                    ...
                    merged, _ = crossmatch(astropy.table.vstack(list(self.catalogs[2][freq].values())), 0.5*utils.arcmin)
    ...
    def build(self, catalogs, seperation, cuts=[('snr','>5')], verbose=True, overwrite=False):
        #common_cols = get_common_colnames(misc.NestedDictValues(catalogs))
        common_cols = None
        structure = {}
        for freq in catalogs.keys():
            ...
            for season in catalogs[freq].keys():
                ...
                if  os.path.exists(target_file) and not overwrite:
                    ...
                    catalogs[freq][season] = _postprocess_catalog(astropy.table.vstack(list(catalogs[freq][season].values())))
            ...
            if  os.path.exists(target_file) and not overwrite:
                ...
                #misc.insert_to_dict(self.hc, cat, ['master', freq, season]) 
                for season in catalogs[freq].keys():
                    astropy.io.ascii.write(catalogs[freq][season], self.output_path('{}_{}.csv'.format(freq, season)))
